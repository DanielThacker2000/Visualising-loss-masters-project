...
class Plotter:
    ...
    def load_images(self, function_name, noise_type, std_list, strategy):
        """
        ...
        for std in std_list:
            std_str = str(std)
            dir_path = os.path.join(self.base_path, function_name, noise_type, std_str, strategy)
    ...
    def plot_images(self, function_name, noise_type, std_list, strategy):
        """
        ...
        - strategy (str): The strategy name.
        """
        images = self.load_images(function_name, noise_type, std_list, strategy)
...
def create_save_directory(graph_name, function_name, noise_type, std, strategy, base_path):
    ...
    save_path = os.path.join(base_path, function_name, noise_type, std_str, strategy)
...
def plot_ci(x_test, y_test, y_pred, y_ci, coverage, mean_width, name, conformal_method, noise_type,std):
    # for val in range(len(y_ci[:, 0, 0])):
    #     print(y_ci[val, 0, 0]-y_ci[val, 1, 0])
    base_path = "results"
    save_path = create_save_directory("PIs.png", name, noise_type, std, conformal_method, base_path)
...
def coverage_for_each_strategy(file_name, stds, noise_type):
    ...
    for std in stds:
        ...
        for data_name in data['data_name'].unique():
            ...
            save_path = create_save_directory("coverage_against_strategy.png", data_name, noise_type, std, "pointless",base_path)
            plt.savefig(save_path)
            # Show the plot
            plt.show()
            
def coverage_for_each_std(file_name, noise_type):
    data = pd.read_csv(file_name)
    ...
    # Iterate over each unique 'data_name'
        for data_name in data['data_name'].unique():
            ...
            save_path = create_save_directory("coverage_against_std.png", data_name, noise_type, "pointless", stategy,base_path)
            plt.savefig(save_path)
            # Show the plot
            plt.show()