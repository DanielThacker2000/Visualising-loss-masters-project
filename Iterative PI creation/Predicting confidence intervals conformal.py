...
#GENERATE DATA
out_file_name = "activ_func.csv" #REMEBER TO CHANGE THE FILENAME
...
active_functions = ['relu', 'tanh', 'logistic'] #
if real_data:
                            ...
                            for bagging_frac in bagging_fractions:
                                for num_features in num_features_arr:
                                    for active_function in active_functions:
                                        ...
                                        total_iterations = len(num_samples_array)*len(layer_size_array)*len(training_data_names)*len(noise_types)*len(dataset_names)*len(cv_sizes)*len(optimisers)*len(bagging_fractions)*len(num_features_arr)
                                        ...
                                        #The MLP
                                        mlp_regressor = MLPRegressor(hidden_layer_sizes=layer_size, verbose=False, random_state=42, early_stopping=True, solver=optimiser, activation=active_function) #200,1000
