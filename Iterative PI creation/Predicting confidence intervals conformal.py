...
#GENERATE DATA
out_file_name = "testering.csv" #REMEBER TO CHANGE THE FILENAME
...
optimisers = ["adam","sgd", "lbfgs"]
#bagging_fractions = [0.1,0.3,0.5] # already done "1"
bagging_fractions = [0.1]
                        ...
                        for optimiser in optimisers:
                            for bagging_frac in bagging_fractions:
                                ...
                                if os.path.exists(out_file_name):
                                    ...
                                    if real_data:
                                        existing_df = pd.DataFrame(columns=["datset","strategy", "data_name", "coverage", "mean_width", "cwc", "mwi","total_time","layer_size","ssc","cv_size", "optimiser", "bagging_fraction"])
                                    else:
                                        
                                        existing_df = pd.DataFrame(columns=["strategy", "data_name", "coverage", "mean_width", "cwc", "mwi","total_time","noise_std","num_samples","noise_type","layer_size","ssc","cv_size", "optimiser", "bagging_fraction"])
