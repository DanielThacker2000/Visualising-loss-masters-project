...
low = 0
high = 1
#np.random.uniform(low, high, (n_features, n_samples))
def generate_multi_linear_data(x_train_dict,y_train_dict,n_samples, n_features, std=0.1, coef_low=0.001, coef_high=1):

    
    np.random.seed(42)
    X = np.random.uniform(low, high, (n_samples, n_features))
...
def generate_multi_polynomial_data(x_train_dict,y_train_dict,n_samples, n_features, std=0.1, coef_low=0.001, coef_high=1):
...
def generate_nonlinear_data(x_train_dict,y_train_dict,n_samples, n_features, std=0.1, coef_low=0.001, coef_high=1):
    ...
    for i in range(n_features):
        if i % 3 == 0:
            y += np.sin(X[:, i:i+1] * coefficients[i])
        elif i % 3 == 1:
            y += np.exp(X[:, i:i+1] * coefficients[i])
        else:
            y += np.log(X[:, i:i+1] * coefficients[i] + 1)
...
def generate_data_multi(num_samples=300,num_features=6, stds=[0.3,0.5,0.7,1],noise_types=["norm","student-t","mixture"], function="linear_multi"):
    ...
    for noise_type in noise_types:
        if function =="poly_multi":
            for std in stds:
                x_train_dict, y_train_dict = generate_multi_polynomial_data(x_train_dict,y_train_dict,num_samples, num_features, std, coef_low=0.001, coef_high=1)
        elif function =="linear_multi":
            for std in stds:
                x_train_dict, y_train_dict = generate_multi_linear_data(x_train_dict,y_train_dict,num_samples, num_features, std, coef_low=0.001, coef_high=1)
        elif function =="non_linear_multi":
            for std in stds:
                x_train_dict, y_train_dict = generate_nonlinear_data(x_train_dict,y_train_dict,num_samples, num_features, std, coef_low=0.001, coef_high=1)
        else:
...
# data_calib, data_test, A_theoretic = gen_gaussian(100,100,3,1,'zero','matern-1/2')
# for i in range(3):
#     plt.hist(data_calib[:,i])
#     plt.show()

# x_train_dict= {}
# y_train_dict = {}
# num_features = 5
# x_train_dict, y_train_dict = generate_data_multi(num_samples=300,num_features=5, stds=[0],noise_types=["norm","student-t","mixture"], function="linear_multi")
x_train_dict1, y_train_dict1 = generate_data_multi(num_samples=300,num_features=5, stds=[0],noise_types=["norm","student-t","mixture"], function="poly_multi")
# x_train_dict2, y_train_dict2 = generate_data_multi(num_samples=300,num_features=5, stds=[0],noise_types=["norm","student-t","mixture"], function="non_linear_multi")

# # x_train_dict, y_train_dict = generate_multi_linear_data(x_train_dict,y_train_dict,300, 6, std=0.1, coef_low=0.001, coef_high=1)

# for i in range(num_features):
#     plt.scatter(x_train_dict[0][:,i],y_train_dict[0])
#     plt.show()
#     # plt.hist(x_train_dict[0][:,i])
# for i in range(num_features):
#     plt.scatter(x_train_dict1[0][:,i],y_train_dict1[0])
#     plt.show()
#     # plt.hist(x_train_dict1[0][:,i])
# for i in range(num_features):
#     plt.scatter(x_train_dict2[0][:,i],y_train_dict2[0])
#     plt.show()
    # plt.hist(x_train_dict2[0][:,i])
