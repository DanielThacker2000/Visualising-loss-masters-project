...
class Plotter:
    ...
    def plot_images_from_dir(self, dir_path,title_list, im_width=12,im_height=4.8,cols=2,titles=1):
        ...
        for idx, (param, img) in enumerate(images):
            ...
            ax.imshow(img, aspect='auto')
            if titles:
                ax.set_title(f"{param}", fontsize=18, pad=0)
            ax.axis('off')  # Show the axes
        #plt.title("Comparing optimiser against different multivariate functions", fontsize=16, pad=0)
        plt.show()
...
def plot_cat_plot(df, x_var, y_var, col_var, col_labels=None, y_limits=None):
    g = sns.catplot(
    df, kind="bar",
    x=x_var, y=y_var, col=col_var,
    height=4, aspect=.5, errorbar=("pi", 50), capsize=.4,
    err_kws={"color": "0", "linewidth": 1.5},
    )
    # Set custom column labels if provided
    if col_labels:
        for ax, label in zip(g.axes.flat, col_labels):
            ax.set_title(label)
    ...
    if y_limits:
        for ax in g.axes.flat:
            ...
            if y_var =="coverage":
                ax.axhline(1-alpha, ls="--", color="r")
    plt.show()

def plot_bar_plot_hue(df, x_var, y_var, col_var, col_labels=None, y_limits=None, y_var_title=None):
    specific_order_layer = ['(200, 1000)', '(400, 2000)', '(200, 1000, 1000, 1000)', '(400, 2000, 2000, 2000)']
    if x_var == "layer_size":
        df = df.copy()
    ...
    ax = sns.barplot(
    df,
    x=x_var, y=y_var, hue=col_var,color="mistyrose",
    errorbar=("pi", 50), capsize=.4,
    err_kws={"color": "0.7", "linewidth": 1.5},
    )

    # if col_labels:
    #     handles, _ = ax.get_legend_handles_labels()
    #     ax.legend(handles=handles, labels=col_labels, title="Function")
    #For label OUTSIDE OF GRAPH
    if col_labels:
        handles, _ = ax.get_legend_handles_labels()
        ax.legend(handles=handles, labels=col_labels, title="Function",
                  bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
    if y_var_title:
        ax.set_ylabel(y_var_title)
    if y_limits:
        ax.set_ylim(y_limits)
    if y_var =="coverage":
        ax.axhline(1-alpha, ls="--", color="r")
...
def plot_box_multi(df1,df2, data_name1, hue1, x_var,
                                data_name2, hue2,x_lab, y_var1,y_var2,y_lab1, y_lab2,
                               title=None):
    if not data_name1:
        filtered_df1 = df1
    ...
    if y_var1 == y_var2:
        y_var1_y_lim, y_var1_y_upper =  get_y_lims(y_var1)
        y_var2_y_lim, y_var2_y_upper =  get_y_lims(y_var2)
        #["Split","JK+","JK+ab","CQR","RNS"]
    # Function to plot coverage and mean width
    def plot_graph(ax, filtered_df, hue, y_var, y_lab):
        # Positions of the bars on the x-axis
        
        if y_var1 != y_var2:
            y_var1_y_lim, y_var1_y_upper =  get_y_lims(y_var)
        if y_var1 == y_var2:
            y_var1_y_lim, y_var1_y_upper =  get_y_lims(y_var1)
        bar_positions = range(len(filtered_df))
        ...
        # if x_var == "data_name":
        #     ax.set_xticklabels(["Linear","Polynomial","Non-linear"], fontsize=16, rotation=15)
        # else:
        #     ax.set_xticklabels(filtered_df[x_var], fontsize=16)
        ax.tick_params(axis='y', labelsize=16)
        if x_var =="layer_size":
        ...
        if y_var =="coverage" or y_var =="cwc":
            ...
            ax.set_ylim(y_var1_y_lim*0.9, y_var1_y_upper*1.1)
        if x_var == "strategy":
                ax.set_xticklabels(["Split", "JK+", "JK+ab", "CQR", "RNS"], fontsize=16)
        if y_var == "total_time":
            ax.set_yscale('log')
        ax.grid(True)
        ax.legend( fontsize=16)
