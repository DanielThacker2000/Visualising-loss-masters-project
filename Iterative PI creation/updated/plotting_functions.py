...
def create_save_dir_real(graph_name, dataset, strategy, layer_size, base_path):
    
    # Construct the directory path
    save_path = os.path.join(base_path, dataset, strategy, layer_size)
...
def plot_coverage_along_quantiles(conformal_method,y_test,y_pred,lower_bound,upper_bound,x_test,title, method, layer_size_str):
    error = y_pred-lower_bound
    fig, axs = plt.subplots(1,1)
    warning1 = y_test > y_pred+error
    warning2 = y_test < y_pred-error
    warnings = warning1 + warning2
    y_test_covered = y_test[warnings]
    y_test_missed = y_test[~warnings]
    
    num_quantile = 10
    x_test['quantile'] = pd.qcut(x_test.index, num_quantile, labels=False)

    # Initialize a list to store the percentage of 'false' values for each quantile
    false_percentages = []
    
    # Loop through each quantile
    for q in range(num_quantile):
        # Get the boolean array indicating which rows are in the current quantile
        quantile_mask = x_test['quantile'] == q
        
        # Get the corresponding warnings values for the current quantile
        quantile_warnings = warnings[quantile_mask]
        
        # Calculate the percentage of 'false' values in the current quantile
        false_percentage = np.mean(quantile_warnings == False)
        false_percentages.append(false_percentage)
    
    x_test.drop(columns='quantile', inplace=True)
    
    fig, ax = plt.subplots(1,1)
    ax.bar(np.arange(0,num_quantile),false_percentages)
    ax.set_xlabel("Quantile")
    ax.set_ylabel("Average coverage")
    ax.set_title(f"Coverage across {title} dataset quantiles")
    ax.grid()
    ax.axhline(1-alpha, ls="--", color="k")
    
    base_path = "Real_dataset_results"
    save_path = create_save_dir_real("Real_pis_cov_along_x.png", title, method, layer_size_str, base_path)
    plt.savefig(save_path)
    plt.show()
    d = {"dataset" : title,"coverage":np.array(false_percentages),"method": method}
    df=pd.DataFrame(d)
    save_path_csv = create_save_dir_real("Real_pis_cov_along_x.csv", title, method, layer_size_str, base_path)
    df.to_csv(save_path_csv, index=False)
    #split dataset into quantiles
    #Get average coverage for each quantile and plot


def plot_ci_real_data(method,y_test_sorted,y_pred_sorted,lower_bound,upper_bound,coverage,width, title, layer_size_str):
    
    
    # lower_bound = np.array(lower_bound)
    # y_pred_sorted = np.array(y_pred_sorted)
    # y_test_sorted = np.array(y_test_sorted)
    # total_points =  len(y_test_sorted) - 1 
    # random_points = np.random.randint(0,total_points,50)
    # lower_bound_ = lower_bound[random_points]
    # y_pred_sorted_ = y_pred_sorted[random_points]
    # y_test_sorted_ = y_test_sorted[random_points]
    ...
    axs.errorbar(
        ...
        alpha=0.5,linestyle="None",marker=".",
    ...
    axs.errorbar(
        ...
        y_pred_sorted_[warnings],
        yerr=np.abs(error[warnings]),
        color="red",alpha=0.5,linestyle="None",marker=".",
    ...
    axs.set_title("Predicted against target value for {title} dataset")
    #axs.set_title(f"Predicted values using the {method} method on {title}. Coverage {coverage:.2f}, MPIW {width:.2f}, Layer Size {layer_size_str}")
    base_path = "Real_dataset_results"
    save_path = create_save_dir_real("Real_pis.png", title, method, layer_size_str, base_path)
    plt.savefig(save_path)
    plt.show()
    ...
    axs1.set_xlabel("True target value")
    axs1.set_ylabel("Prediction interval width")
    axs1.set_title("Prediction interval width against target value for {title} dataset")
    save_path = create_save_dir_real("Real_pis_width.png", title, method, layer_size_str, base_path)
    plt.savefig(save_path)
    plt.show()
