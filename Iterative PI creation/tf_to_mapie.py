...
class TensorflowToMapie():
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        self.history = self.model.fit(
            X_train, y_train, 
            batch_size=64,
            epochs=20, verbose=0,callbacks=[early_stopping_monitor],
        )
...
def get_model(input_shape, activation_func="relu",optimiser="adam", layer_size = (200, 1000)):
    """
    ...
    model.add(Dense(layer_size[0], activation=activation_func, input_shape=input_shape))
...
# x_train["sinex_con"], y_train["sinex_con"] = gen.generate_data(1000, [0.1],"normal","sinex_con")
# x_test["sinex_con"], y_test["sinex_con"] = gen.generate_data(1000, [0.1],"normal","sinex_con")


# mapie_model = MapieRegressor(estimator=tf_wrapped_model, method="plus", cv="split", random_state=42) 
# mapie_model.fit(x_train["sinex_con"][0.1].ravel(), y_train["sinex_con"][0.1].ravel())
# #%%
# y_preds, y_pss = mapie_model.predict(x_test["sinex_con"][0.1].ravel(), alpha=0.1)

# #%%
# print(y_preds.ravel())
# print(y_pss.ravel())
# print(x_train["sinex_con"][0.1].ravel())