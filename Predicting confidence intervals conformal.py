...
def train_and_run_ci(model,x_train,y_train,x_test,y_test, alpha, function_name,conformal_method, param, real_data, noise_type,std,num_samples):
    ...
    if real_data:
        ...
        plotting.plot_ci(x_test, y_test, y_pred, y_ci, coverage, mean_width, function_name, conformal_method, noise_type, std,num_samples)    
    
    #save to results/STRATEGY/std
    return coverage,mean_width, cwc, y_ci, total_time
#%%
#GENERATE DATA
out_file_name = "test_results.csv"
...
for training_data_name in training_data_names:
    for noise_type in noise_types:
        ...
        match noise_type:
            ...
            case "student":
                stds = [4,5,10,1000] #student t
            case "cauchy":
                stds = [0.01,0.05,0.15,0.3] #Cauchy std
            case "laplace":
                stds = [0.5,1,1.5,2] #Laplace
            case _:
                stds = [100000] #normal
        # stds = [0,0.2,0.5,1] #block normal
        # stds = [3,5,10,1000] #student t
        # stds = [0,0.05,0.15,0.3] #Cauchy std
        # stds = [0.5,1,1.5,2] #Laplace
        ...
        #The MLP
        mlp_regressor = MLPRegressor(hidden_layer_sizes=(500,1000,1000,1000), verbose=False, random_state=42, early_stopping=True, solver="adam") #200,1000
        ...
        if os.path.exists(out_file_name):
            ...
            if real_data:
                existing_df = pd.DataFrame(columns=["strategy", "data_name", "coverage", "mean_width", "cwc", "total_time"])
            else:
        ...
        #Plot and calculate real data graphs
            ...
            for strategy, params in STRATEGIES.items():
                ...
                coverage,mean_width, cwc, y_ci, total_time = train_and_run_ci(
                    mlp_regressor,x_train,y_train,x_test,y_test, alpha, "yacht data",strategy, params, real_data, noise_type, 0,"string"
                    )
                results.append({
                    "strategy": strategy,
                    "data_name": file_name,
        ...
        if plot_coverage_width_strategies:
            plotting.coverage_for_each_strategy(out_file_name,stds, noise_type,num_samples_str)
        if plot_coverage_width_stds:
            plotting.coverage_for_each_std(out_file_name, noise_type,num_samples_str)
